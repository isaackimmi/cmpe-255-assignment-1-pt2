...
The standard-library backend completes on CPU in under a second and reports validation and untouched held-out test character loss/perplexity plus OOV counts and a sample. The default protocol is a strict chronological 80/10/10 character split. Boundary context is carried from the preceding ground-truth prefix into each suffix; targets within a suffix are teacher-forced. The test suffix is scored only after training/model selection.
...
- **Results/artifacts:** `metrics.json` records train/validation/test sizes, a train-only vocabulary with OOV accounting, validation/test metrics, corpus hash, split offsets, runtime configuration, and environment metadata; the unittest suite covers split validation, OOV normalization, boundary context, CLI validation, and the causal mask when Torch is installed.
